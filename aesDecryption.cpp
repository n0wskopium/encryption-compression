...
using namespace std;
// Inverse S-Box lookup table for AES decryption
...
{
    if (encryptedBlock.size() != 16 || key.size() != 16)
    {
    ...
    for (int col = 0; col < 4; col++)
    {
        for (int row = 0; row < 4; row++)
        {
            state[row][col] = encryptedBlock[col * 4 + row];
        }
    }
    vector<uint8_t> expandedKey = keyExpansion(key);
    addRoundKey(state, expandedKey, 10); // Starting with round 10 key
